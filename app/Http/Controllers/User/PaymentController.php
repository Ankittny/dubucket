...
class PaymentController extends Controller
{
    ...
    public function OnlinePaytm(Request $request){
        ...
        $transaction_id = $request->razorpay_payment_id;
        $order_result = $this->orderStore($user, $total_price, $totalProduct, 'online', $request->transactionid , 1, $shipping, $shipping_fee, $coupon_price, 1, $request->billing_address_id, $request->shipping_address_id);

        $this->sendOrderSuccessMail($user, $total_price, 'Prepaid', 1, $order_result['order'], $order_result['order_details']);
    ...
    public function orderStore($user, $total_price, $totalProduct, $payment_method, $transaction_id, $paymetn_status, $shipping, $shipping_fee, $coupon_price, $cash_on_delivery,$billing_address_id,$shipping_address_id){
        ...
        foreach($cartProducts as $key => $cartProduct){
            ...
            $product = Product::select('id','price','offer_price','weight','vendor_id','qty','name','thumb_image','unitkg')->find($cartProduct->product_id);
            $price = $product->offer_price ? $product->offer_price : $product->price;
            $price = $price + $variantPrice;
            $isFlashSale = FlashSaleProduct::where(['product_id' => $product->id,'status' => 1])->first();
            $today = date('Y-m-d H:i:s');
            ...
            $orderProduct->thumb_image = $product->thumb_image;
            $orderProduct->unitkg = $product->unitkg;
        ...
        $orderAddress->pincode = $shipping->pincode;
    ...
    public function myfatoorah_webview(Request $request) {
        // try {
            ...
            $amount_real_currency = $total_price;
            $myfatoorah = MyfatoorahPayment::first();
            $price = $amount_real_currency * $myfatoorah->currency_rate;
            $price = sprintf('%0.2f', $price);
