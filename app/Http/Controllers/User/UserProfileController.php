...
class UserProfileController extends Controller
{
    ...
    public function order(){
        $user = Auth::guard('api')->user();
        $orders = Order::orderBy('id','desc')->where('user_id', $user->id)->orderBy('id','desc')->get();
    ...
    public function onTheWayOrder(){
        $user = Auth::guard('api')->user();
        $orders = Order::orderBy('id','desc')->where('user_id', $user->id)->where('order_status',2)->get();
        return response()->json(['orders' => $orders]);
    }
    ...
    public function myProfile(){
        $user = Auth::guard('api')->user();
        $personInfo = User::select('id','name','email','dob','phone','image','country_id','state_id','city_id','zip_code','address')->find($user->id);
