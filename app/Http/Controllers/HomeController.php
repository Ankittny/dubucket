...

{
    ...
    public function subCategoriesByCategory($id){
        $categories = Category::where(['slug' => $id, 'status' => 1])->first();
        if(!empty($categories)){
            $subCategories = SubCategory::where(['category_id' => $categories->id, 'status' => 1])->get();
    ...
    public function productByCategory($id){
        ...
        if(!empty($category)){
            $products = Product::with('activeVariants.activeVariantItems')->where(['category_id' => $category->id, 'status' => 1,'approve_by_admin' => 1])->select('id','name', 'short_name', 'slug', 'thumb_image','qty','sold_qty','short_description','long_description','price','weight','unitkg','offer_price','is_undefine','is_featured','new_product', 'is_top', 'is_best','category_id','sub_category_id','child_category_id','brand_id')->orderBy('id','desc')->get();
            return response()->json(['category' => $category, 'products' => $products]);
        }
        $subcategory = SubCategory::where('slug',$id)->first();
        if(!empty($subcategory)){
            $products = Product::with('activeVariants.activeVariantItems')->where(['sub_category_id' => $subcategory->id, 'status' => 1,'approve_by_admin' => 1])->select('id','name', 'short_name', 'slug', 'thumb_image','qty','sold_qty','short_description','long_description', 'price','weight','unitkg','offer_price','is_undefine','is_featured','new_product', 'is_top', 'is_best','category_id','sub_category_id','child_category_id','brand_id')->orderBy('id','desc')->get();
            return response()->json(['SubCategory' => $subcategory, 'products' => $products]);
        }else {
            return response()->json(['category' => null, 'products' => []]);
        }
...

    {
        ...
        $bestProducts = Product::with('activeVariants.activeVariantItems')->select('id','name', 'short_name', 'slug', 'thumb_image','qty','sold_qty', 'price','weight','unitkg','offer_price','is_undefine','is_featured','new_product', 'is_top', 'is_best','category_id','sub_category_id','child_category_id','brand_id','long_description')->where(['is_best' => 1, 'status' => 1, 'approve_by_admin' => 1])->orderBy('id','desc')->get()->take($bestProductVisibility->qty);
    ...
    public function product(Request $request){
        ...
        $products = $products->select('id','name', 'short_name','long_description','slug', 'thumb_image','qty','sold_qty', 'price','unitkg','offer_price','is_undefine','is_featured','new_product', 'is_top', 'is_best','category_id','sub_category_id','child_category_id','brand_id');
