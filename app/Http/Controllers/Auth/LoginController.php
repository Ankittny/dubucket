...
class LoginController extends Controller
{
    ...
    public function googleRegister(Request $request){
        $user = User::where('email',$request->email)->select('id','name','email','phone','image','status','fcm_token','deviceid','password')->first();
        ...
        if(empty($user)){
            ...
            $userData->deviceid = $request->deviceid;
            $userData->status = 1;
            ...
            $token = Auth::guard('api')->attempt($credential, ['exp' => Carbon::now()->addDays(365)->timestamp]);
            
            $user1 = User::where('id',$userData->id)->select('id','name','email','phone','image','status','fcm_token','deviceid','password')->first();
            return $this->respondWithToken($token,1,$user1);
        } else {
             $credential=[
                'email'=> $request->email,
                'password'=> $request->password
             ];
             //echo print_r($credential); die;
